-- =============================================
-- SELLAR MOBILE APP - STORAGE POLICIES
-- Migration 11: File upload and storage policies
-- =============================================
...
BEGIN
    -- Clean up orphaned profile image files (profiles that no longer exist)
    ...
    -- Clean up orphaned listing image files (listings that no longer exist)
    DELETE FROM storage.objects 
    WHERE bucket_id = 'listing-images'
    AND NOT EXISTS (
        SELECT 1 FROM listings 
        WHERE user_id::TEXT = (storage.foldername(storage.objects.name))[1]
    );
    
    GET DIAGNOSTICS deleted_count = deleted_count + ROW_COUNT;
    
    -- Clean up old verification document files (older than 2 years)
    DELETE FROM storage.objects 
    ...
    -- Clean up old chat attachment files (older than 1 year)
    DELETE FROM storage.objects 
...
CREATE INDEX idx_storage_usage_total_size ON storage_usage(total_size);
...
-- Success message
SELECT 'Storage policies and functions created successfully!' as status;
