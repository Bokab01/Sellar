...
export default function PersonalizedRecommendationsScreen() {
  ...
  const sections = useMemo<SectionData[]>(() => {
    ...
    if (trendingRecommendations.length > 0) {
      sectionArray.push({
        ...
        data: createPairs(trendingRecommendations)
      });
    }
    ...
    return sectionArray;
  }, [personalizedRecommendations, trendingRecommendations, recentlyViewed, theme.colors.primary]);
  ...
  const RecommendationCard = React.memo(({ listing }: { listing: RecommendationListing }) => {
  ...
  const renderItem = useCallback(({ item: pair }: { item: RecommendationListing[] }) => {
    return (
      <View style={{ flexDirection: 'row', marginBottom: theme.spacing.sm, paddingHorizontal: theme.spacing.xs }}>
        {pair.map((listing) => (
          <RecommendationCard key={listing.listing_id} listing={listing} />
        ))}
